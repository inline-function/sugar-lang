...
number : INT | DEC;
//匿名对象
anonymousObject : OBJECT (':' type+)? ('{'NL* (variable|function) (NL variable|function)* NL* '}')?;
//顶层语句
...
stmt :  expr | top ;
//表达式
expr : (number | STRING | '(' NL* expr NL* ')' | ID | lambda | anonymousObject) name? invoke? assign? infix?;
//赋值语句(右递归文法)
assign : NL* '=' NL* expr;
//匿名函数(可能是右递归文法)
...
//调用语句(右递归文法)
invoke : typeArgList? '(' NL* (NL* expr (NL* ',' NL* expr)*)? NL* ')' lambda?;
//中缀调用法(右递归文法)
infix : ID NL* expr;
//名称访问语句(右递归文法)
name : NL* '.' ID;